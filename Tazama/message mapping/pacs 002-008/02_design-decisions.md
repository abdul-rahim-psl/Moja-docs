...
## D9 — `GrpHdr.MsgId` / `CreDtTm`: copy when supplied 🆕 *revises FSD §6.5*

FSD v1.1 states these are "generated by the PPA itself on every outbound message — they are not mapped from Mojaloop inputs". **The golden path contradicts this:** msg 17's `extensionList` supplies both:
...
**Rule: use the scheme-supplied value when present in `extensionList`; generate a ULID / use the PPA construction timestamp otherwise.** Preserves traceability between the Tazama message and the Mojaloop message that produced it.

In practice: **pacs.002 copies** (msg 17 supplies both); **pacs.008 generates** (msg 16 supplies neither).
