...
Reviewing any pull request in the `psl-izyane-cch-frms` rule-processor fleet — a separate set of repos from `cch-mla`/`cch-ppa`, one per Tazama rule — follows its own working instructions, not this file's story/QA workflow: [`izyane-PR-review/Claude_PR_Review_Template.md`](izyane-PR-review/Claude_PR_Review_Template.md). Read it in full before starting any such review; it covers preflight, the rule-processor-specific hunts (inert config parameters, the fleet-wide message-type constraint, band/unit checks, query correctness), the required file format, and the settled facts that are not re-litigated per PR.
