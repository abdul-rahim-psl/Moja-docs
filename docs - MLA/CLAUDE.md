...
- **Why-comments are short — 1-2 lines, one reason, every time new code is written, not just on a cleanup pass.** `engineering-rules.md` §5 caps this explicitly after the pattern recurred: a multi-paragraph JSDoc block re-litigating alternatives tried, chaining through `core-knowledge.md`/`qa-review-findings.md`/other files' own comments, or narrating how something was live-verified. That reasoning belongs in the story's `executive-summary.md`, not inline. This applies to every new comment written from here on, not only to comments already flagged for shortening — the instruction to shorten existing comments is not a one-time pass to revert to old habits on the next story.
- **This is production code, not an internal audit trail — comment like a senior engineer writing for the next engineer, never for the process that produced the change.** Never put a QA finding number, ticket ID, PR reference, or any other artefact of *how* the change came about into a comment — a comment states the durable engineering reason the code is shaped the way it is (an invariant, a constraint, a trade-off), not that it fixes something or where that something is tracked. `engineering-rules.md` §5 has the binding rule; this recurred once already (F-11/F-12 comments citing `qa-review-findings.md` by number) and was corrected — do not reintroduce the pattern on a later finding.

## Reviewing a rule-processor PR (`psl-izyane-cch-frms`)

Reviewing any pull request in the `psl-izyane-cch-frms` rule-processor fleet — a separate set of repos from `cch-mla`/`cch-ppa`, one per Tazama rule — follows its own working instructions, not this file's story/QA workflow. Read, in order: [`izyane-PR-review/Claude_PR_Review_Template.md`](izyane-PR-review/Claude_PR_Review_Template.md) (preflight, the rule-processor-specific hunts, the required file format, the settled facts not re-litigated per PR); [`izyane-PR-review/calibration-notes.md`](izyane-PR-review/calibration-notes.md) (16 pattern-library entries and 19 standing rules the template itself doesn't yet state, each grounded in a specific PR's verified evidence — not optional background, applied as first-class checks on every review); [`izyane-PR-review/review-queue.md`](izyane-PR-review/review-queue.md) (what's already been reviewed, what's queued, and which of the two phases — calibration or independent review — is currently active). **One PR is reviewed per session turn, only when explicitly told to proceed** — never work ahead through the queue on your own initiative, and never conflate studying an existing review (calibration) with producing an independent one (the real workload) unless `review-queue.md` says which phase is active.
