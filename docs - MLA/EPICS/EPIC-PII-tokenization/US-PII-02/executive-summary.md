...
Three further items were filed under `plan.md` §7.1 #2–#4 / §13.2, "gates production, not the work ahead" — a different, lower-stakes category than the fail-mode item, and explicitly *not* blocking this story's own completion once the fail-mode answer landed:

- ~~Secret rotation strategy~~ **Resolved [2026-09-18], spec confirmed [2026-09-22]: no rotation, a long-lived key** (`docs/meetings and emails/tokenization-feedback.md`; `plan.md` §16's "gate item #2 reversed" entry). No rotation mechanism is needed — `FilePiiSecretClient`'s existing "restart to change" design already matches the requirement exactly.
