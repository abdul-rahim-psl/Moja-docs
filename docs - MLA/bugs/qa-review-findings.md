...
| F-12 | [x] | Medium | `isConnected()` never flips on broker disconnect; readiness lies | **Fixed and live-verified.** `KafkaClient`'s constructor now subscribes to kafkajs's own `CONNECT`/`DISCONNECT`/`CRASH` consumer events and tracks `connected` from them, not from its own `connect()`/`disconnect()` call boundaries. Live-verified by stopping the real broker under a running instance: `/health/ready` flipped to and stayed `kafka: "DOWN"` for the whole outage (previously stayed `"UP"` throughout), then recovered to `"UP"` on its own once the broker returned. Full write-up: `plan.md` §16's [2026-09-23] entry. |
| F-13 | [x] | Medium | `PPA_BASE_URL` path prefix dropped | **Fixed and live-verified.** `HttpsPpaClient` now extracts and prepends `baseUrl`'s/`healthBaseUrl`'s own path prefix to every request, independently for delivery and health-probe traffic; a query string or fragment on either URL is now a fatal boot error. Live-verified against a real path-prefix-stripping proxy in front of the real PPA: 8/8 canonical envelopes delivered and accepted through the prefixed path, versus a confirmed 404 on the same proxy's unprefixed route. Full write-up: `plan.md` §16's [2026-09-24] entry. |
| F-14 | [x] partial | Medium | No explicit TLS min version / `rejectUnauthorized` / keep-alive agent; certs not reloadable | **Fixed and live-verified, scoped to connection reuse and explicit TLS policy.** `HttpsPpaClient` now builds one reused `https.Agent`/`http.Agent` per destination (delivery, health probe) at construction, with `keepAlive`, a bounded `PPA_MAX_SOCKETS`, and (for mTLS) explicit `minVersion: 'TLSv1.2'`/`rejectUnauthorized: true`. Live-verified: connection count to PPA stayed at 3 across 16 deliveries (no growth), confirming reuse; a real park/breaker-trip/recovery cycle confirmed the separate health-probe agent still works correctly. **Certificate hot-reload and the expiry metric are deliberately left out** - hot-reload is blocked on CCH confirming the rotation mechanism (US-SEC-01, not yet answered); the expiry metric is a separate, smaller follow-up. Full write-up: `plan.md` §16's [2026-09-24] entry. |
