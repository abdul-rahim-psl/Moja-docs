...
### F-23 — Critical — Health probe can't pass real PPA's mTLS, so a tripped partition never resumes
...
### F-24 — High — Kafka startup failure is never retried; liveness stays UP forever
...
### F-25 — High — PII secret is read once; the PII reprobe path can never heal; a pod without its secret still takes partitions
...
### F-26 — High — Secret content is never validated: empty file becomes an unkeyed hash; trailing newline changes every token
...
### F-27 — High — Kafka connection has no TLS/SASL support, and the question has never been asked
...
### F-28 — High — Party PII outside the tokenize table reaches PPA/TMS in cleartext on QUOTE events
...
### F-29 — Medium — No rebalance handling: stale reprobe loops keep running after a partition is reassigned
...
### F-30 — Medium — Top-level `catch` is an unmetered, unalerted drop path
...
### F-31 — Medium — Typo'd topic name silently auto-creates an empty topic and reports fully healthy
...
### F-32 — Medium — Watchdog `retrigger` can spawn a second, concurrent reprobe loop on the same partition
...
### F-33 — Medium — `complexName` tokens depend on JSON key order
...
### F-34 — Medium — `plan.md` §16's F-17 entry misdescribes the commit-retry recovery path
...
### F-35 — Medium — URL scheme is never cross-checked against `PPA_MTLS_DISABLED`
...
### F-36 — Medium — Every post-connect transport error is labelled a TLS handshake failure
...
### F-37 — Low — JSON parse errors leak PII fragments into the error log
...
### F-38 — Low — Shutdown cancels park timers before handlers finish draining
...
### F-39 — Low — Internal finding IDs left in `src/` code and a Prometheus help string
...
### F-40 — Low — Party-lookup traffic is metric-labelled as `egress`, not `party-lookup`
...
### F-41 — Low — Type/comment claims contradicted by real captures
...
### F-42 — Low — Pod runs with no Kubernetes `securityContext`
...
### Q-01 — One file threads the same 12 dependencies through five hand-written shapes
...
### Q-03 — A domain abstraction now generalizes over exactly one domain
...
### Q-04 — A circular (type-only) import between the consumer and its logging module
...
### Q-05 — Skip/rejection dispatch isn't exhaustive by construction
...
### Q-06 — One 1,821-line test file covers the entire consumer
...
### Q-07 — Operator-facing text uses internal vocabulary; comments exceed the length cap
