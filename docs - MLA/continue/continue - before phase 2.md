...
- [x] **Payload selection per D6.** **Done [2026-09-02]** — `src/services/payload-selection.service.ts`, `selectPayload` returns `transformedPayload ?? payload` (one expression, correct for every event type without branching on `eventType`); returns `undefined` rather than `{}` for a missing body, deliberately diverging from the POC so an empty body feeds the item below instead of being silently forwarded. No `data:` URI decode built — D6 makes it available-on-demand, and nothing yet names a need for it. 8 new Jest tests against real captures.
- [x] **Unreadable-record path**: log, alert, advance the offset. Never retried. **Done [2026-09-02]** — `src/services/audit-record-parser.service.ts`, `parseAuditRecord`: raw Kafka value → typed record or a named `unreadable` outcome with a reason. Extends the POC's `parseAuditMessage` with a structural shape check the POC skipped, so a record missing `metadata.trace.tags` is caught here rather than crashing three layers downstream. **The log/alert/advance-offset behaviour itself is not wired** — same position as item 5 above: a pure predicate the eventual handler will call, not a handler. 8 new Jest tests. **123 tests total (was 43 at the start of this phase), 100% statements/lines/functions, 99.27%+ branches, all seven checklist items now built.**

**Integration — done, live-verified [2026-09-02].** `src/services/ingestion.service.ts` (`processRecord`, the pure pipeline chaining all seven decisions above in order) and `src/services/ingestion-consumer.service.ts` (`createIngestionHandler`, the I/O wrapper: distinct-per-reason logging, then an unconditional offset advance), wired into `src/index.ts`. 17 new tests (140 total), 100%/100%/100%/98.76%+.

**Live-verified against the real harness.** Started the real built service against local Redpanda, fresh consumer group, fed the 41-record partition-2 slice: **all 41 accounted for** (23 egress, 3 party-lookup, 15 forwarded across all four event types), zero errors. `SIGKILL`'d it, restarted under the identical group: zero reprocessing of the 41, then fed a fresh 20-record corridor and confirmed the restarted instance picked it up and categorized it correctly. Full numbers: `plan.md` §5 and §16.

**Not done:** a new golden target for this phase's classified/selected output (`tools/golden` still only diffs the raw topic, Phase 1's concern) - noted as optional in §8 below, genuinely not built. **Found, not addressed:** ~38 pre-existing orphaned `node build/index.js` processes on the verification machine, unrelated to this session's work, left untouched and flagged to the user.

**The Phase 2 exit criterion (§6 below) is now substantially met** - live-proven above, except the golden-file comparison.
