...
- [ ] **Missing / invalid signature ⇒ security log, alert, advance the offset.** Not retried — same shape as Phase 2's `unreadable` path, one layer further in. *(Logging is wired — `SECURITY`-marked error log, offset advances unconditionally — but no real alert channel exists yet, Phase 6; left unchecked pending that or an explicit call that the interim logging is enough for this phase.)*
