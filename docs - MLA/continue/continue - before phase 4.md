...
This is [`plan.md`](../plan.md) §7, US-PII-01 and US-PII-02. Nothing on it is built yet.

- [ ] **Field classification per the Fields-to-Tokenize table, per event type** (core-knowledge.md §4.1). QUOTE and FXQUOTE bodies only — TRANSFER/FXTRANSFER bodies carry PII exclusively inside the ILP packet, which is exempt (below), so there is nothing to tokenize in those two event types' bodies at all; confirm this against real captures rather than assuming it.
- [ ] **ILP-carried fields explicitly exempt.** The exemption is the *whole* decoded ILP packet, not a per-field allowlist inside it — cryptographically bound into `condition`; rewriting any of it breaks the transfer. `payload-selection.service.ts`'s own module comment already explains why no ILP decode exists in this codebase (D6, available-on-demand only) — this phase must not become the reason one gets built speculatively; tokenization operates on the FSPIOP-form `body` `selectPayload` already returns, and that body never contains a decoded ILP packet for the event types this phase actually touches.
- [ ] **Transaction amounts never tokenized, in any message.** A regression case, not just a positive one: prove tokenization does not touch `amount`, even coincidentally, on every event type it runs against.
- [ ] **Keyed hashing (never a bare hash), deterministic, with a recognizable token prefix.**
- [ ] **Secret loaded once at startup from a mounted location; readiness reflects load success.** Mirrors the public-key store's own startup-load pattern (Phase 3) but, unlike that store, must actually gate `/health/ready` — a new coupling `health.service.ts` does not have yet.
- [ ] **The ordering test:** a test that fails if tokenization is moved ahead of signature validation. `engineering-rules.md` calls this out as enforced by a test, not a convention — write it against the real composed pipeline (`envelope-pipeline.service.ts`), not a hand-rolled call sequence that could drift from what production code actually does.
- [ ] **Tokenization-failure metric and alert, distinct from any other failure counter** — named explicitly in `plan.md` §7 because, per the open fail-mode question above, this may end up being the *only* visible signal that PII reached PPA unprotected.
