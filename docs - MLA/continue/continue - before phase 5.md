...
- [x] **mTLS client configuration**; stable service-name addressing, never individual replica addresses. *Built, tested, live-verified — `src/clients/ppa.client.ts`'s `HttpsPpaClient`, the `PpaClient` port. Live-verified against a real running `ppa-stub`: a correct cert (success), a genuinely untrusted cert (tls-handshake-failure), an unreachable host (network-error), a 4xx, and a 5xx via fault injection — each produced the correctly classified outcome. Worth reading before touching this file again: the TLS-handshake classification went through two wrong designs before landing on the right one, each ruled out by this exact live check, not by inspection — see `ppa.client.ts`'s own comment on `classifyTransportError`. Now wired into `ingestion-consumer.service.ts` for offset-gating and the per-call timeout below; retry and breaker logic still do not consume it yet.*
- [~] **Per-call timeout, configured independently of the retry budget.** *Built, unit-tested (not yet live-verified) — `HttpsPpaClient.deliver` races an `AbortController`-driven `setTimeout(this.timeoutMs)` against the whole call (headers and body drain both), returning a distinct `{ outcome: 'timeout', timeoutMs }` result rather than folding a breach into `server-error`/`network-error` — preserving it as its own diagnosable reason, the same treatment `tls-handshake-failure` already gets. Classified by tracking the timer's own firing directly (`stage.timedOut`), never by parsing the resulting error's code/message, mirroring `classifyTransportError`'s own established discipline one function up. `PpaConfig.timeoutMs` was already independent of the retry budget's own fields.*
- [~] **Offset advances only on HTTP 200.** This is the load-bearing change this phase makes — replacing `ingestion-consumer.service.ts`'s current unconditional `kafka.advance(...)` with a call gated on the delivery client's own response. *Built, unit-tested (not yet live-verified) — `createIngestionHandler` now calls the injected `PpaClient.deliver` for every `forwarded` outcome and advances only on `success`. Every other outcome logs its own reason and calls `kafka.pause(partition)`, withholding the advance — deliberately uniform across all five non-success outcomes for now, since the 4xx-advance exception, the retry burst, and the circuit breaker are the checklist's own next items below, not built yet. Pausing (not merely skipping the advance) is the deliberate minimum even at this stage: without it, a later record on the same partition could still commit an offset past this one's unacknowledged failure. `HttpsPpaClient` is now constructed and injected at the composition root (`src/index.ts`).*
