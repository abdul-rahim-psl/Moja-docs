...
- [ ] **4xx ⇒ log the full envelope, alert, advance. Permanent, never retried.** *Half live-confirmed: a real 4xx from `ppa-stub` produced exactly one delivery attempt, live — `isPpaTransient` correctly keeps `client-error` out of the retry loop — but it still pauses instead of advancing, since that half is not built yet.*
