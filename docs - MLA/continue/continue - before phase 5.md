...
This is [`plan.md`](../plan.md) §8, US-MLA-06 and US-MLA-07. **Everything on it is now built, tested, and live-verified — Phase 5 is done [2026-09-07].** Left as a historical record of the checklist this document originally handed off; a Phase 6 handoff document is what future sessions should read for "what's next" (`strategy.md` §2.5's own convention).

- [x] **Endpoint selection by `eventType` per D4** — the routing table above, read from the envelope's own `eventType` field, never re-derived from anything else. *Built, tested, live-verified — `src/services/ppa-routing.service.ts`'s `resolvePpaEndpoint`. Now called from the consumer's own delivery path, via `HttpsPpaClient.deliver`'s own use of it.*
- [x] **mTLS client configuration**; stable service-name addressing, never individual replica addresses. *Built, tested, live-verified — `src/clients/ppa.client.ts`'s `HttpsPpaClient`, the `PpaClient` port. Live-verified against a real running `ppa-stub`: a correct cert (success), a genuinely untrusted cert (tls-handshake-failure), an unreachable host (network-error), a 4xx, and a 5xx via fault injection — each produced the correctly classified outcome. Worth reading before touching this file again: the TLS-handshake classification went through two wrong designs before landing on the right one, each ruled out by this exact live check, not by inspection — see `ppa.client.ts`'s own comment on `classifyTransportError`. Now wired into `ingestion-consumer.service.ts` for offset-gating, the per-call timeout, and the retry burst below; breaker/re-probe logic still does not consume it yet.*
- [x] **Per-call timeout, configured independently of the retry budget.** *Built, tested, live-verified — `HttpsPpaClient.deliver` races an `AbortController`-driven `setTimeout(this.timeoutMs)` against the whole call (headers and body drain both), returning a distinct `{ outcome: 'timeout', timeoutMs }` result rather than folding a breach into `server-error`/`network-error`. Classified by tracking the timer's own firing directly (`stage.timedOut`), never by parsing the resulting error's code/message. **Live-verified**: `ppa-stub` set to hang forever (`POST /control {mode:"timeout"}`) against a real running MLA and real broker produced four genuine HTTPS requests roughly 2000ms apart (before each retry's own backoff) — the configured budget enforced per call, live.*
- [x] **Offset advances only on HTTP 200.** This is the load-bearing change this phase makes — replacing `ingestion-consumer.service.ts`'s current unconditional `kafka.advance(...)` with a call gated on the delivery client's own response. *Built, tested, live-verified — `createIngestionHandler` calls the injected `PpaClient.deliver` for every `forwarded` outcome and advances only on `success`. A transient result is retried in place (below) before falling through; once exhausted — or immediately for the one permanent outcome (4xx) — the outcome's own reason is logged and `kafka.pause(partition)` is called, withholding the advance — deliberately uniform across every case that reaches this point for now, since the 4xx-advance exception and the circuit breaker are the checklist's own next items below, not built yet. **Live-verified across several fault-injection rounds** (timeout, persistent 503, 4xx, an untrusted client cert) against a real `ppa-stub` and real broker, with re-signed real captured records: every non-200 outcome left the offset unadvanced and paused the partition (kafkajs's own "Pausing fetching" log each time); **a full MLA restart against the still-paused partition redelivered the exact same parked offset**, twice (once recovering from the timeout fault, once from the untrusted-cert fault), which then advanced cleanly with nothing lost once the fault cleared. The automatic re-probe that would make this immediate rather than restart-dependent is still not built (§ below).*
- [x] **5xx / timeout / TLS-handshake failure ⇒ retry ×3, exponential backoff (1s/2s/4s) with genuinely random jitter, offset not advancing.** *Built, tested, live-verified — `runPpaRetryBurst`, mirroring the PII secret's own burst (§2 above) in the other direction: up to `PPA_MAX_RETRIES` further attempts (default 3), the same `computeBackoffMs` jitter under 1s/2s/4s ceilings, offset withheld throughout. **`network-error` is retried here too, alongside the three the story names explicitly** — reasoned, not guessed: treating PPA being wholly unreachable as more final than an ordinary 5xx would silently lose events, the exact failure mode this mechanism exists to prevent. Worth raising back to CCH/COMESA if a future FSD revision disagrees. The loop re-checks transience every attempt, so a retry that itself turns permanent (4xx) stops the burst immediately rather than spending remaining attempts on it. **Live-verified twice** over real HTTP round-trips: a hung connection produced 4 real requests spaced 2739ms/3943ms/2772ms apart; a persistent 503 produced 4 real requests spaced 864ms/1417ms/1508ms apart — each gap comfortably inside its own 1s/2s/4s ceiling and visibly different from the others, live, not only under a mocked clock.*
- [x] **TLS handshake failure treated as transient, with the underlying reason preserved in the alert.** *Built, tested, live-verified — a `tls-handshake-failure` result is retried by the same burst as a 5xx (R-22), and the existing per-outcome alert (§ above, built the prior session) still names the handshake and its reason specifically once the burst gives up, never folded into a generic 5xx message. **Live-verified with a genuinely untrusted client cert** — a throwaway, unrelated self-signed CA, not the harness's own — presented to the real, running `ppa-stub` (`rejectUnauthorized: true`): the connection reset with "socket hang up", retried through the full burst, and the alert read "PPA TLS handshake failed at ...: socket hang up - pausing partition" — never a generic "HTTP 5xx." Restoring the correct cert and restarting redelivered and forwarded the same parked record cleanly.*
- [x] **4xx ⇒ log the full envelope, alert, advance. Permanent, never retried.** *Built, tested, live-verified — `logPpaPermanentRejection` logs the full serialized envelope (not just its id/eventType — US-MLA-07's own AC wording), and the caller advances immediately, never pausing, whether the very first attempt is a 4xx or a retry turns permanent mid-burst. **Live-verified**: a real 4xx from `ppa-stub` produced exactly one delivery attempt and `PPA rejected the envelope at partition 2 offset 497 with HTTP 400 - permanent, advancing. Envelope: {...}` — the full envelope in the log, including its tokenized fields (`tkn_...` prefixes visible — live confirmation Phase 4's tokenization survives this path unchanged) — no pause, no retry.*
- [x] **Retry exhaustion and circuit breaking as two coordinated mechanisms.** Exhaustion parks the event and keeps retrying it on a timer; those failures accumulate toward a configurable N; at N the breaker trips and pauses the whole affected partition, re-probing on a timer. *Built, tested, live-verified — `parkAndReprobePpa` parks and pauses on burst exhaustion, feeding `PpaCircuitBreaker` — **one independent counter per partition**, deliberately not process-wide like the PII secret's own breaker (a reasoned scope decision, not a verbatim requirement — `core-knowledge.md` §3.5's "the affected partition(s)" wording is plural/selective; `plan.md` §16's US-MLA-07 entry has the full argument, flagged to confirm with CCH/COMESA). The shared counting primitive (`CircuitBreaker`, generalized out of what was previously the PII-only `PiiCircuitBreaker`) is unchanged; only the per-partition wrapper is new. **Live-verified**: a persistent 503 against a real running instance produced the park alert, then, after four further failed reprobes, exactly one `Circuit breaker tripped for partition 2: 5 consecutive PPA delivery failures` — confirmed not repeated across further continued failure.*
- [x] **Every pause paired with a re-probe that can resume it.** A partition paused with no path back is the exact bug the POC's own breaker existed to fix (`plan.md` §12). *Built, tested, live-verified — the detached reprobe loop resolves a reprobe three ways (success; a `client-error` surfacing now that PPA is reachable again — itself proof of connectivity, so it resumes and resets the breaker exactly like a success does; or still transient); a failed offset-advance on a recovered reprobe retries the commit next tick rather than a bespoke policy; an unhandled exception inside one reprobe tick is caught and does not stop the loop. **Live-verified, and this is the one that matters most**: restoring `ppa-stub` to healthy after a tripped breaker — **with no process restart at all** — produced, on the very next reprobe tick, `PPA recovered at partition 2 offset 498 - circuit breaker reset, resuming partition 2`, `Forwarded QUOTE (id=01KZRP0MH81MYFTW7PH0S9SYF2) at partition 2 offset 498`, and kafkajs's own `Resumed partition 2` — the exact parked record, nothing lost or duplicated, recovered entirely on its own. The qualitative difference from every recovery proof above (each needed a restart): this one didn't. Per-partition isolation is unit-verified only — the one real capture fixture in this repo is entirely partition 2.*

**Exit criterion — live. Met [2026-09-07].** Against `ppa-stub`: a 503 leaves the offset unadvanced and the event is redelivered on recovery ✓; three 5xx responses produce three backed-off retries with visibly different jitter ✓ (864ms/1417ms/1508ms, live); N consecutive failures trip the breaker and pause consumption ✓ (5 consecutive, live); restoring the stub resumes from the paused event with nothing lost or duplicated ✓ (live, no restart); a 4xx advances immediately ✓ (live, full envelope logged); a TLS handshake failure retries and its alert names the handshake, not a generic 5xx ✓ (live). **Phase 5 is done.** Full narrative: `plan.md` §16's US-MLA-06/US-MLA-07 entries; epic documentation at `EPICS/EPIC-3-delivery-to-ppa-offset-management/`.
