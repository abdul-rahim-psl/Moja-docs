...
**What Phase 6 is not starting from a blank slate on, and what it is.** `pino` structured logging has been in place since Phase 0; every log line already carries a `serviceOperation` field naming the pipeline step. What does **not** yet exist, despite `engineering-rules.md` §9 and `plan.md` §9 both naming it as a from-the-first-line-of-code requirement: **no log line in this codebase includes `correlationId`**, and **no `eventType` field is attached at the structured-logging layer either** — both are read straight from the interpolated message text where present at all (`at ${partition} offset ${offset}`, `Forwarded ${eventType}...`), never as their own indexed field. Ack latency is not measured anywhere — there is no clock started at consumption and stopped at commit or delivery. None of this is a surprise buried in the code; it is stated plainly here because `plan.md` §15's own sequencing note warned this would be far more expensive to retrofit than to have built in from the start, and the honest thing to do is say so rather than let Phase 6 discover it mid-build.
...
5. [`../plan.md`](../plan.md) §9 — the actual Phase 6 checklist, which this document walks through but does not replace. Also `plan.md` §13.1's own row naming `cch-crosscutting-user-stories.md` (absent — US-AUD-01, US-MON-01/R-37, US-MON-02, US-PERF-01) as blocking this phase **in full**, not just alerting destinations.
...
- **`cch-crosscutting-user-stories.md` is referenced throughout the requirements but the document itself is absent** (`plan.md` §13.1) — it is the source for US-AUD-01 (audit-log PII masking), US-MON-01 (monitoring, including R-37's alerting-destination gap), US-MON-02 (readiness scoping — already built correctly, instance-local only, per `engineering-rules.md` §8), and US-PERF-01 (the ≤200ms p95 ack-latency budget this phase must measure against). **This gates Phase 6 in full**, per `plan.md`'s own row — not just the alerting-destination half of it. Obtain the document, or raise this explicitly before assuming the acceptance criteria this phase needs are fully known.
- **Alerting destinations themselves (R-37)** — Slack, PagerDuty, email, a webhook, something else entirely. Build the paths against a configurable sink; do not guess a destination and wire it in as if decided.
- **What "real" observability infrastructure looks like in the target environment** — Prometheus/Grafana, a Tazama-adjacent stack, something COMESA already runs — is not stated anywhere in the knowledge base read for this handoff. Worth asking rather than assuming a specific metrics library or wire format is already implied.

---

## 5. The Phase 6 checklist

This is [`plan.md`](../plan.md) §9. Nothing on it is built yet.

- [ ] **Structured logging (`pino`) with `correlationId`, `eventType` and pipeline step on every line — from the first line of real code, not retrofitted.** *Partially true today, stated precisely, not overclaimed: `pino` and a `serviceOperation` field (the pipeline step) have been in place since Phase 0. `correlationId` and `eventType` are not yet their own structured field anywhere — see §1 above for the specific, verified gap. "Not retrofitted" is no longer available as a description; this phase's own job is exactly that retrofit, honestly.*
- [ ] Metrics for every question an operator must answer without a debugger: throughput and consumer lag; skip counters **by reason** (`egress`, party-lookup, unclassifiable, FX-quote-rejected, unreadable); rejection counters (missing/invalid signature, key-source outage, 4xx); tokenization failures; retry and breaker state (both breakers, per §3 above); delivery outcomes (success/client-error/server-error/timeout/tls-handshake-failure/network-error — `PpaDeliveryResult`'s own six-way union already gives this its exact shape); ack latency against the 200ms p95 budget (US-PERF-01, still gated on the absent crosscutting document per §4 — the *mechanism* to measure latency can be built against a stated default budget regardless, the same posture every prior phase took toward an unconfirmed value).
...
- **Do not let "structured logging already exists" read as "this checklist item is already done."** `pino` being wired in since Phase 0 is necessary, not sufficient — `correlationId` and `eventType` are not yet their own field on any log line in this codebase, confirmed by inspection (§1 above), not assumed. Grep before claiming otherwise.
...
- **`cch-crosscutting-user-stories.md`'s absence blocks this phase in full, per `plan.md` §13.1 — not merely the alerting-destination half of it.** Confirm what acceptance criteria this phase actually needs to satisfy before assuming the checklist above (drawn from `plan.md` §9's own terse four bullets) is the complete spec; it may not be.
