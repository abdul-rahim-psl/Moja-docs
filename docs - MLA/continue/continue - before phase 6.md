...
**What Phase 6 is not starting from a blank slate on, and what it is.** `pino` structured logging has been in place since Phase 0; every log line already carries a `serviceOperation` field naming the pipeline step. **Resolved [2026-09-08] — §5's first checklist item:** `correlationId` and `eventType` are now their own structured fields, live-verified against the real harness, not merely interpolated into message text; `serviceOperation` on the ingestion path is now five granular pipeline-step values instead of one blanket string. ~~**Still genuinely open:** ack latency is not measured anywhere — there is no clock started at consumption and stopped at commit or delivery.~~ **Resolved [2026-09-08] — §5's second checklist item.** `receivedAt` is captured at the top of `createIngestionHandler` and observed into `mla_ack_latency_ms` on every path that reaches a genuine PPA 200, a parked-then-recovered record included; the 200ms p95 *budget* against that histogram stays Phase 7's own load-test claim, not this phase's. None of this was a surprise buried in the code; it was stated plainly here because `plan.md` §15's own sequencing note warned it would be far more expensive to retrofit than to build in from the start, and the retrofit is exactly what §5's first two items now record as done.
...
This is [`plan.md`](../plan.md) §9. **All four items below are now built, tested, and live-verified**, and the phase's own exit criterion is met — see `plan.md` §16's US-MON-01/US-PERF-01 entries and [`EPICS/PHASE-6-Observability-Operability/`](../EPICS/PHASE-6-Observability-Operability/) for the full record, not this superseded document.
...
- [x] **Metrics for every question an operator must answer without a debugger.** *Built, tested, live-verified [2026-09-08].* `prom-client` added (chosen explicitly over its designated successor `@prometheus-io/client`, which is pre-1.0 with a fraction of the adoption — a real trade-off, not a default). A `Metrics` port (`metrics.interface.ts`, named methods mirroring `Logger`'s own shape, not a generic string-keyed dispatch) plus its adapter (`clients/metrics.client.ts`, the only file importing `prom-client`) implement all ten named signals: `mla_skipped_total{reason}`, `mla_rejected_total{reason}` (JWS/envelope/4xx, all five-plus-one permanent reasons), `mla_tokenization_failures_total`, `mla_forwarded_total{event_type}`, `mla_ppa_delivery_outcomes_total{outcome}` (one per real HTTP round trip, retries included), `mla_pii_breaker_state` (one gauge, process-wide), `mla_ppa_breaker_state{partition}` (one gauge **per partition** — the two breakers' deliberately different shapes, both now real metrics, not just prose), `mla_partition_paused{partition}` (this codebase's own stated reading of US-MON-01's undefined "paused-offset-rate" term — a state gauge, not a literal rate, flagged as an interpretation rather than guessed silently), `mla_consumer_lag{partition}` (broker high-water mark minus this consumer group's own **committed** offset, not just in-memory fetch position — deliberate, since N1 lets fetch position run ahead of what is durably processed), and `mla_ack_latency_ms` (a histogram, Kafka-consume-to-PPA-200, US-PERF-01's own literal definition, bucketed around the 200ms budget). `/metrics` (`fastify.client.ts`) serves it in Prometheus text-exposition format alongside `collectDefaultMetrics`'s own process/event-loop signals. 42 new/changed tests (dedicated `metrics.client.test.ts` exercising the real `prom-client` registry, not a mock, plus the ingestion suite's own metrics describe block), 339 tests total, 100%/97.97%/100%/100% coverage, zero lint errors.
  - **A real, live-consequence bug this work found and fixed, not just plumbed around**: wiring `mla_forwarded_total` exposed that a record recovering from a PII-secret park (`parkAndReprobe`'s own reprobe) advanced the Kafka offset and logged "Forwarded" **without ever calling `ppaClient.deliver`** — `attempt()`/`buildEnvelopeFromKafkaValue` builds an envelope, it never delivers one, and only the main handler's own body used to run a `forwarded` outcome through PPA at all. A PII outage that recovered via reprobe silently skipped PPA delivery entirely: offset advanced (so Kafka never redelivered it), "Forwarded" logged (so nothing looked wrong). Undetected because no test ever asserted `ppaClient.deliver` was called on that specific path. **Fixed** by factoring the full delivery/retry/park classification into one shared `resolveOutcome`, used by both a fresh record and a PII-recovered one — a PII recovery can no longer bypass PPA. A second, subtler defect surfaced *while fixing the first*: naively resuming the partition immediately on PII recovery (before confirming the record was actually delivered) would let kafkajs fetch new messages on the same partition while `resolveOutcome` might still be handing the record to `parkAndReprobePpa`'s own separate park — breaking the one-reprobe-loop-per-partition invariant both mechanisms rely on. Fixed by having `resolveOutcome` report whether it actually settled the record, and resuming only then.
  - **Live-verified precisely** (Redpanda + `ppa-stub`, a genuinely re-signed real `postQuotes` record, real mTLS): with the PII secret file genuinely removed, the record retried, parked, and tripped the breaker (`eventType":"QUOTE"` on the park/trip log lines, live). **The restart-based recovery path** — restoring the secret and restarting under the identical consumer group, the only PII-recovery path actually exercisable live today, the same limitation §2/gate-item-#2 has named since Phase 4 (`FilePiiSecretClient` has no hot-reload; a live, no-restart reprobe recovery for the PII secret specifically stays unit-verified only, not a new gap this phase introduced) — redelivered the exact same parked offset, and this time it **genuinely reached `ppa-stub` over real mTLS** (`received.jsonl` gained the QUOTE envelope, `id`/`correlationId` matching the live log). The `/metrics` snapshot immediately after showed `mla_forwarded_total{event_type="QUOTE"} 1`, `mla_ppa_delivery_outcomes_total{outcome="success"} 1`, `mla_ack_latency_ms_sum 34` (well inside the 200ms budget), `mla_rejected_total{reason="invalid-signature"} 6`, `mla_skipped_total{reason="egress"} 10`/`{reason="party-lookup"} 1`, `mla_pii_breaker_state 0`, and real `mla_consumer_lag{partition="N"} 0` for all twelve partitions computed from the live broker's own admin API — every named metric proven against real data in one run, not asserted only in a unit test.
- [x] **A metric for every decision the code makes silently.** *Verified complete as a byproduct of the item above, [2026-09-08], not a separate build.* Every skip/rejection reason `core-knowledge.md` §9's own "what is deliberately dropped" table names for MLA (`egress`, party-lookup, unclassifiable, unreadable, fx-quote-rejected, missing/invalid signature, key-source outage, incomplete/invalid envelope, a PPA 4xx) now has exactly one counter incrementing it, unconditionally, regardless of log level — including the debug-level structural skips (`egress`/`party-lookup`) that are never logged above debug. Swept for gaps deliberately, not assumed: no MLA-side decision point was found without a corresponding metric call.
- [x] Alert paths wired for: missing/invalid signature, 4xx, retry exhaustion, breaker trip, tokenization failure. **Sink configurable, destination undecided** (§4 above). *Built, tested, live-verified [2026-09-08] — an `Alert` port (`src/interfaces/alert.interface.ts`, five named methods, severity fixed per method) and its `WebhookAlertClient` adapter (`src/clients/alert.client.ts`), with **two real sinks**: `mla_alerts_total{type,severity}` (always active — what a Prometheus/Alertmanager rule watches, needing no destination decided) plus an optional webhook (`AlertConfig.webhookUrl`, unset by default, fire-and-forget, `AbortController`-bounded). Wired at exactly the five points this file's own §1 and `ingestion-consumer.service.ts`'s module comment had marked as interim log lines; `key-source-unavailable` deliberately raises none of them. The wiring pushed `ingestion-consumer.service.ts` past ESLint's `max-lines` gate, so its log/meter/alert leaf dispatch moved to a new `src/services/ingestion-outcome-logging.service.ts` — a pure relocation, no behaviour change. All five conditions fired live against the real harness on genuinely re-signed records. 354 tests, 100%/98.06%/100%/100%, zero lint errors.*

**Exit criterion — live. Met [2026-09-08].** A full 500-record feed (`raw_export_500/raw_export_500.json`) against a fresh instance and a real `ppa-stub` produced a `/metrics` snapshot accounting for every record in exactly one bucket: `mla_skipped_total{reason="egress"} 273` + `{reason="party-lookup"} 92` + `{reason="fx-quote-rejected"} 19` + `mla_rejected_total{reason="invalid-signature"} 116` = **500** ✓; `mla_consumer_lag` 0 on all twelve partitions ✓; no partition left paused ✓; zero unhandled exceptions ✓; `mla_alerts_total{type="signature",severity="failure"} 116`, matching every counted rejection ✓.
...
- ~~**Ack latency has no clock anywhere yet.** This is new instrumentation, not a value already computed and merely unexposed — do not assume a `Date.now()` call already exists near the consumer's own entry point; it does not (verified by grep, §1 above).~~ **Resolved [2026-09-08] — §5's second checklist item.** The clock exists now (`receivedAt`, `createIngestionHandler`) and feeds `mla_ack_latency_ms`. The trap this bullet warned about no longer applies; it stays here, struck through, as the record of what to check before trusting a similar-looking claim about a different signal.
