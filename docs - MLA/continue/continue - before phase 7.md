...
- [x] Sustained load via `--loop`, measured against 25 TPS sustained / 125 TPS peak, with ack latency against the 200 ms p95 budget — including tokenization overhead, which US-PII-01 requires be confirmed under load rather than assumed. *Built, tested, live-verified [2026-09-09] — see `plan.md` §10 for the full entry.* `tools/load-test/` built and checked in. All three US-PERF-01 shapes pass live: 25 TPS sustained x 30 min (10440/10440 within 200ms, lag 0 at all fifteen samples), 125 TPS peak x 5 min (8702/8702 within budget, lag 0), and the 125->25 step-down (6,000 fed = 6,000 accounted, zero event loss). Mean ack latency 1.6-2.8ms against a 200ms budget.
- [x] Two MLA instances against 12 partitions — group rebalance, no double-processing, no gaps. *Built, tested, live-verified [2026-09-09] — see `plan.md` §10 for the full entry.* Rebalance confirmed (`STATE Stable, MEMBERS 2`, 12 partitions split 6/6 round-robin); the 500-record export fed **once** produced exactly the single-instance baseline set — no duplicates, no gaps — split A=66/B=50 so neither instance was idle. Also survived a mid-feed `SIGKILL` of one instance with all 116 keys still arriving. Two things worth carrying forward: the identity key is **`id` + `msgType`**, not `id` alone (116 envelopes over only 75 distinct `id`s, per D3/D4 — deduping on `id` would report 41 false duplicates), and after killing an instance you must poll the **broker-side** `rpk group describe` TOTAL-LAG, not per-instance `/metrics`, which cannot see the dead member's partitions until the session timeout expires.
- [x] Chaos: broker restart mid-feed; MLA `SIGKILL` mid-dispatch; stub flapping. *Built, tested, live-verified [2026-09-09] — see `plan.md` §10 for the full entry.* **Broker restart:** MLA reconnected on its own; 500 produced = 500 accounted for, nothing lost. **MLA `SIGKILL`:** killed with 28 of 116 delivered; all 116 arrived after restart, no gaps — real resume-from-committed-offset. **Stub flapping:** 102 transient failures and 6 retry exhaustions parked records and paused two partitions; clearing the fault recovered all of them automatically, no restart, reaching 116/116 in 30s. Zero duplicates throughout, but **not** claimed as exactly-once — MLA is at-least-once by design (N1) and a redelivery would be correct. **`mla-restart` is now `runnableNow: true`; all 15 scenarios in the library are runnable.** One trap recorded: identify the MLA process by the port it owns, never `pgrep -f 'build/index.js'` — this box runs ~41 unrelated `build/index.js` processes from the Tazama stack and the pattern selects one of those.
