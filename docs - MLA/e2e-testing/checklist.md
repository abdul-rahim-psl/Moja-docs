...
- [ ] **[remote-instance]** Each of the 8 envelopes from §1's run is actually present in the deployed PPA's write-ahead store, byte-for-byte — an HTTP 200 alone only proves PPA *claimed* to persist it. Still open — this is specifically about the *remote* `10.0.115.186:3000` instance, which the local-stack run below has no visibility into.
...
- [ ] **[local-stack / Tazama]** Confirm the resulting pacs.008 carries the cached FX enrichment (`InstdAmt`/`XchgRate`, §7.3's "From cached FX Quote" row) — still open. Not reachable via the 2026-09-21 run specifically because `pacs.008` failed local schema validation before dispatch (see §3.6's now-confirmed-live finding) — there is no dispatched message to inspect until that gap closes.
