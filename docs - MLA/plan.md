...
| **Phase 4 — PII tokenization** | **Mechanism built, tested, and live-verified, [2026-09-04]; gate item #1 (fail-mode) also built, tested, and live-verified, [2026-09-04]; gate item #2 (secret rotation) answered and resolved [2026-09-18, spec confirmed 2026-09-22]** — §16 (US-PII-01/02, plus US-PII-01's and US-PII-02's own follow-up entries), §7. **Both gates that blocked formal closure are now answered — no rotation, a long-lived key, needing no further build. Formal closure itself (exit-criterion writeup, epic docs) has not yet been done as a distinct step.** |
...
**Open before go-live:** what "protected" must legally mean, and named ownership of the secret. Both are CCH decisions — §13. (The rotation strategy, gate item #2, is resolved — see below.)

**Status as of the mechanism's build: not formally closed, per §7.2, as a distinct step — but both gating decisions are answered.** Everything above is built, tested above the coverage gate, and live-verified. Gate item #1 (fail-mode) is built, tested, and live-verified, and no longer gates anything. **Gate item #2 (secret rotation) is also resolved [2026-09-18, spec confirmed 2026-09-22]** — COMESA reversed its earlier "versioned keys" direction to a long-lived, non-rotating key (rotation would break Tazama's own fraud-rule matching on the same MSISDN/bank-account values across transaction history), needing no build — `FilePiiSecretClient`'s existing design already matches it. `EPICS/EPIC-PII-tokenization/` executive-summary/file-register documents are written and current as of gate item #1's resolution; the story's own formal closure (finalizing those documents against gate item #2's resolution too, and the exit-criterion writeup) has not yet been done as a distinct step, per `CLAUDE.md`'s "External decisions" rule and this section's own §7.2.
...
1. **Can we have the DFSP public keys, or a JWKS endpoint?** Without them, JWS verification cannot be proven against real traffic — only against fixtures we sign ourselves. This is the single highest-value unblock available. **Answered in part [2026-09-09 meeting, `docs/meetings and emails/9-sept.md`]** — see §13.1's DFSP-keys row for the full outcome (key inventory confirmed, Infotex retrieval and an MCM onboarding video pending). **Gates Phase 3** (§13.1) until the keys/JWKS/video actually arrive.
