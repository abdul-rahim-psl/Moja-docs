...
**All five were put to CCH and the Mojaloop Foundation at the 2026-09-09 meeting** (`docs/docs - MLA/meetings and emails/9-sept.md`). Answers are summarized against each question below; `plan.md` §13.1 and §14 carry the full, maintained record — this file is kept as what was actually asked, not re-derived from the answers.

1. ~~**Can we have the DFSP public keys, or a JWKS endpoint?**~~ **Dissolved [2026-09-23] — no longer needed:** MLA no longer validates DFSP signatures (`e2e-testing/remove-JWS.md`, confirmed by all parties), so it needs no keys. Original question: Without them, JWS verification cannot be proven against real traffic — only against fixtures we sign ourselves. This is the single highest-value unblock available. **Answered in part, not yet delivered:** the 19 ids resolve to 8 DFSPs, 2 FXPs and 9 regional hubs; George is obtaining the keys via Infotex and checking for a JWKS endpoint. Separately, MLA should interface with Mojaloop Connection Manager (MCM) rather than hold its own key store — Sam to share an onboarding video.
...
along with it. Both sign-offs — removing US-MLA-05 from scope and retiring `engineering-rules.md` N3 — are
confirmed. Michael's question back ("what use case are they guarding against?") is being left unanswered
by decision: his point that nothing reaches MLA unvalidated was accepted as sufficient.
