...
**Current state:** Phases 0 through 6 are built and live-verified, and **Phase 7 (hardening and validation) is development-complete and live-verified but not formally closed** — all six of its checklist bullets are done (load against the 25/125 TPS baseline and the 200 ms p95 budget, two-instance rebalance, chaos, and all 15 named scenarios passing unattended from a cold start via `npm run scenario:all`), with its exit criterion open on one clause, "in CI", blocked on a runner constraint rather than on engineering (`plan.md` §10, §16). All of this lives in [`cch-mla`](/home/abdul-rahim/mojaloop/cch-mla) — the separate repository this knowledge base governs; nothing under `docs - MLA/` runs code. Phase 4's mechanism is built and live-verified too, though the phase is not yet formally closed. A real ingestion pipeline exists, and on top of it, real envelope construction, real JWS verification, real PII tokenization, real delivery to PPA with the full offset/retry/breaker/reprobe mechanism, and now full observability and operability: the MLA consumes from a real Kafka broker, classifies each event, verifies the DFSP's signature, tokenizes party-identity fields, builds a schema-valid `EventEnvelope`, and delivers it to PPA over mTLS — committing the offset only on HTTP 200, retrying a transient failure with genuine jitter, logging a permanent 4xx in full and advancing immediately, and parking a still-failing event behind a per-partition circuit breaker that re-probes and resumes entirely on its own — every decision live-verified end to end, including a genuinely re-signed record being accepted by a live `ppa-stub` over mTLS with prefixed tokens in every listed field, a tampered one failing, a stripped signature failing distinctly, a key-source outage failing distinctly from an invalid signature, a missing PII secret failing distinctly again, and a persistent-503 breaker trip recovering automatically once `ppa-stub` health is restored, with no process restart. Every one of these conditions now also raises a real alert — a metrics-based signal always active, plus an optional configurable webhook — rather than only a structured log line. **Phase 4's gate item #1 (the fail-mode wiring, `plan.md` §7.1 #1) is now built, tested, and live-verified. Phase 4 stays open on gate item #2 — secret rotation (`plan.md` §7.1 #2, §13.1) — not on any remaining engineering for item #1, and unaffected by Phase 5 or 6 closing. Phase 5 (delivery to PPA, offsets, and resilience) is done [2026-09-07]** — its exit criterion is fully met live, tracked in `plan.md` §16's US-MLA-06/US-MLA-07 entries. **Phase 6 (observability and operability, `plan.md` §9) is done [2026-09-08]** — its exit criterion (a full 500-record feed accounted for in exactly one bucket, summing to 500) is fully met live, tracked in `plan.md` §16's US-MON-01/US-PERF-01 entries; only R-37 (alerting destination/routing) stays open with CCH, gating no engineering this codebase controls. **Phase 7 (hardening and validation, `plan.md` §10) is now the active work.** See `plan.md` §1 and §16 for what has actually been built, and `continue/continue - before phase 7.md` for the current session handoff. `cch-crosscutting-user-stories.md` (§2.2 below) confirms the observability stack (Prometheus/Grafana/Loki/Tempo/Mimir, IDD §10) and leaves only the alerting destination/routing genuinely open (R-37); `core-knowledge.md` has been cross-referenced against it (its own §5 scope note, §12.2, and §13's open register carry the result). Five user-story documents now exist and have been through a consolidated review; several critical findings are resolved and a smaller set — including one Critical (**R-04**, the two "never synthesize" prohibitions) and one High (**R-37**, alerting routing) — remains open. See `core-knowledge.md` §13.
