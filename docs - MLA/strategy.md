...
**A real PPA instance was confirmed live [2026-09-17]**, reachable at `http://10.0.115.186:3000` — `GET /health/ready` returns `{"ready":true,"checks":{"writeAheadStore":true}}`, and its own OpenAPI doc (`/documentation`, `/documentation/json`; title "PPA Ingress API") lists exactly the four business endpoints and the Event Envelope schema this knowledge base specifies (`core-knowledge.md` §3.4, §5) — `POST /QUOTES`, `/FXQUOTES`, `/TRANSFERS`, `/FXTRANSFERS`, plus `/health/live`/`/health/ready`. Its example payloads use this project's own test DFSP ids (`test-mwk-dfsp`, `test-zmw-dfsp`). **Not yet confirmed:** whether this instance enforces mTLS on the business endpoints (its OpenAPI doc declares no security scheme) or what config would actually point a real MLA at it safely — do not wire MLA to it without confirming that first, since an unvalidated POST could persist into its real write-ahead store (`core-knowledge.md` §6.1 step 2 — persist happens before structural validation).
...
| File | Significance |
| --- | --- |
| **`e2e-testing/checklist.md`** | **The manual, functional happy-path checklist for MLA↔PPA, added [2026-09-17].** Distinct from `tools/scenario-library` (which proves fault/chaos/concurrency paths, `plan.md` §10) and from any unit or golden-file test (which proves one function or one capture's classification, not the live pipeline as a whole): this walks one clean, unmodified corridor transaction through every pipeline stage in order — ingestion, canonical selection, classification, decode, JWS, PII tokenization, envelope construction/schema, mTLS delivery, offset commit — against a real broker and a real `ppa-stub`, with concrete commands, expected log/metric signals, and pass criteria at each stage. §§1–15 target `ppa-stub`, since that is the only PPA-shaped thing this docs folder tracks; **§19 is a separate, additional pass against the real PPA instance confirmed live [2026-09-17]** (§1 above), gated on its own open connection-detail questions rather than run by default. Deliberately excludes every fault/rejection/chaos scenario by scope, listed explicitly at its own close. ~230 lines. |
...
| **Manually proving the core MLA↔PPA happy path works, end to end** | `e2e-testing/checklist.md` in full — not `tools/scenario-library`, which targets fault/chaos/concurrency instead |
