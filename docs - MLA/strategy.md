...
**`user stories/headlines/`** — three short extracts, one per source document group, each holding only that document's first Epic heading, first `US-xxx-xx` story heading, and that story's `Description` paragraph verbatim (nothing else — not Acceptance Criteria, Method, Assumptions or Todos). Not a substitute for §2.2's source documents or `core-knowledge.md`; a quick-glance headline only.

| File | Covers |
| --- | --- |
| **`headlines/MLA.md`** | Epic 1/US-MLA-01's Description from `cch-mla-user-stories.md`, and US-PII-01's Description from `cch-pii-user-stories.md`. |
| **`headlines/PPA.md`** | Epic 6/US-PPA-01's Description from `cch-ppa-user-stories.md`. |
| **`headlines/cross-cut.md`** | Epic 11/US-AUD-01's Description from `cch-crosscutting-user-stories.md`. |
