...
| File | Significance |
| --- | --- |
| **`e2e-testing/checklist.md`** | **The manual, functional happy-path checklist for MLA↔PPA, added [2026-09-17].** Distinct from `tools/scenario-library` (which proves fault/chaos/concurrency paths, `plan.md` §10) and from any unit or golden-file test (which proves one function or one capture's classification, not the live pipeline as a whole): this walks one clean, unmodified corridor transaction through every pipeline stage in order — ingestion, canonical selection, classification, decode, JWS, PII tokenization, envelope construction/schema, mTLS delivery, offset commit — against a real broker and a real `ppa-stub`, with concrete commands, expected log/metric signals, and pass criteria at each stage. States plainly that it exercises MLA against `ppa-stub`, not the real PPA (which does not exist in this workspace yet, §1 above) — it cannot and does not prove anything about PPA's own nine-step pipeline. Deliberately excludes every fault/rejection/chaos scenario by scope, listed explicitly at its own close. ~200 lines. |
