...
**Current state:** Phases 0 through 2 are built and live-verified in [`cch-mla`](/home/abdul-rahim/mojaloop/cch-mla) — the separate repository this knowledge base governs; nothing under `docs - MLA/` runs code. A real ingestion pipeline exists: the MLA consumes from a real Kafka broker with explicit offset control, selects canonical records, classifies event type, detects FX-quote rejections, selects the payload, and handles unreadable records — every decision live-verified end to end, including a genuine mid-feed kill/restart and a checked-in decision-level golden file. **Phase 3 (envelope construction and JWS validation, `US-MLA-04`/`05`) is next** — nothing yet builds an envelope or talks to a PPA. See `plan.md` §1 and §16 for what has actually been built, and `continue/` for the current session handoff. The four user-story documents have been through a consolidated review; several critical findings are resolved and a smaller set — including one Critical (**R-04**, the two "never synthesize" prohibitions) — remains open. See `core-knowledge.md` §13.
...
| **`continue - before phase 3.md`** | **Current.** Phase 2 (the ingestion path) is complete; **Phase 3's envelope construction and JWS validation** (`plan.md` §6, US-MLA-04/05) is the next work. Walks that checklist with the reasoning inline, states plainly that **D3 (the envelope `id` scheme) now gates this phase** and names the action (raise it with PPA's owners), and flags a concrete, previously-unexamined risk in the JWS verification item — whether the captured `content.payload` preserves the exact bytes a DFSP signature was computed over. |
