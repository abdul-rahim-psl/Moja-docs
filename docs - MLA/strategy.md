...
| **`cch-crosscutting-user-stories.md`** | **Pipeline-wide stories not owned by a single component, Epics 11–12 — US-AUD-01, US-MON-01, US-MON-02, US-PERF-01, US-PERF-02, US-SEC-01.** Newly added [2026-09-07]; closes the gap `plan.md` §13.1 and `core-knowledge.md` §13.3/§14 named as blocking Phase 6 in full. Audit-log content and PII masking (US-AUD-01, PPA-side); the seven-signal monitoring spec (US-MON-01) — per-partition consumer lag, paused-offset rate (distinct from lag), degraded-message rate, breaker state at **both** hops, ValKey/TMS-token health as metrics not readiness inputs; health-endpoint scoping (US-MON-02, confirms MLA's existing instance-local design); the 200 ms MLA / 500 ms PPA p95 latency budgets and load-test shape (US-PERF-01); ValKey sizing (US-PERF-02); inter-service mTLS (US-SEC-01). **Confirms the observability *stack*** — Prometheus/Grafana/Loki/Tempo/Mimir, IDD §10, CCH-owned collection agents — settling what `continue - before phase 6.md` §4 had flagged as unstated; **only the alerting *destination/routing* stays open** (**R-37**, High, still Open — PagerDuty/Slack/email and the Alertmanager-style mechanism connecting a condition to it; a SIEM/log-aggregation platform is separately unconfirmed, IDD Open Item #8). Closes with its own findings table (R-37 High; R-10 Medium — the 25/125 TPS baseline cites a superseded IDD version; R-14/R-17/R-36 Low; R-23/R-31 Low/Medium, tracked primarily in the MLA/PPA documents) and a five-row actions table. **`core-knowledge.md` has not yet been cross-referenced against this document** — CLAUDE.md's rule that a new source document triggers that sync, not an assumption it is already reflected; still owed. ~155 lines. |
...
| **`continue - before phase 6.md`** | **Current.** Phase 5 is done — its delivery/offset/retry/breaker/reprobe mechanism is built, tested, and live-verified in full. **Phase 6's observability and operability** (`plan.md` §9) is the active work now; its first checklist item (structured `correlationId`/`eventType`/pipeline-step logging) is done as of [2026-09-08] — built, tested, live-verified against the real harness, including two real defects the retrofit found and fixed (an `eventType` plumbing gap on six skip reasons, and a latent `Extract`-over-a-non-union type bug). Ack latency measurement, metrics, and alert paths remain open. Carries forward gate item #2 (secret rotation, still open, still not blocking) so a fresh session does not lose it. |
