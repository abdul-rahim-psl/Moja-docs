...
| File | Significance |
| --- | --- |
| **`e2e-testing/checklist.md`** | **The manual, functional happy-path checklist for MLA↔PPA, added [2026-09-17].** Distinct from `tools/scenario-library` (which proves fault/chaos/concurrency paths, `plan.md` §10) and from any unit or golden-file test (which proves one function or one capture's classification, not the live pipeline as a whole): walks one clean, unmodified corridor transaction through every pipeline stage in order — ingestion, canonical selection, classification, decode, JWS, PII tokenization, envelope construction/schema, delivery, offset commit — with concrete commands, expected log/metric signals, and pass criteria at each stage. **§19 is the primary path — the real PPA instance (§1 above), by explicit instruction, and already run successfully live [2026-09-17]** (8/8 envelopes delivered and accepted). §§1–18 target `ppa-stub` instead and are kept only for byte-level envelope inspection the real PPA can't offer back. Deliberately excludes every fault/rejection/chaos scenario by scope, listed explicitly at its own close. ~260 lines. |
...
| **Manually proving the core MLA↔PPA happy path works, end to end** | `e2e-testing/checklist.md` §19 (the real PPA, primary) → §§1–18 only if byte-level envelope inspection is needed — not `tools/scenario-library`, which targets fault/chaos/concurrency instead |
