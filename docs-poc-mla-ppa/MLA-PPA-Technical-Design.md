...
**Companion documents:** [`MLA-PPA-Executive-Summary.md`](MLA-PPA-Executive-Summary.md), [`plan-outline.md`](plan-outline.md)
...
**Not resolved by these five captures:** no error, abort, or rejection appears in `DRPP_Kafka_E2E_Pack`'s five transactions — all five settle successfully. **A separate, wider capture (`raw_export_500.json`, 500 records across 12 partitions) has since surfaced real rejection data** — an FX-quote reject, a transfer-prepare reject, and a party-lookup reject, none matching the shape assumed below. See [`rejected-events.md`](rejected-events.md) for the findings and the implementation plan; nothing in this document has been updated to reflect it yet, and every error-handling row in §2.6 and §3.7 below still describes the pre-existing, spec-only state.
...
| Any error callback | Trigger | `pacs.002.001.12` (`TxSts: RJCT`) |
...
| `ABORTED` | FSPIOP | `RJCT` |
| Error callback, any resource | Either | `RJCT` |

`ACSC`, not `ACCC`. `ACCC` asserts funds reached the creditor's account; Mojoloop's `COMMITTED`/`COMM` confirms settlement between schemes, which is not the same claim.
...
**Known interface limitation:** Tazama's `pacs.002.001.12` has no `StsRsnInf` element, so an error callback's `errorCode` / `errorDescription` cannot be carried through. They are retained in the PPA audit log only. The field exists in ISO 20022 generally — this is a Tazama interface gap worth raising as a schema-change request.
...
**No longer in this list, done and live-verified since the version of this document that first wrote it**: the audit-log store, circuit breakers on both hops, and operator-triggered replay of a parked entry — see §5.1 and §7.6–§7.13.
...
| — | **No error, abort, or rejection captured in `DRPP_Kafka_E2E_Pack`'s five transactions — all five settle successfully.** Real rejection data now exists in a separate capture (`raw_export_500.json`); see [`rejected-events.md`](rejected-events.md) for the findings and plan — not yet implemented or reflected below. | Every error-path row in §2.6 and §3.7 still describes the specification-only state, unvalidated against real data. |
...
What these five captures did **not** establish, and this design still relied on the FSD alone for at the time: any error, abort, or rejection path (§2.6, §3.7) — see [`rejected-events.md`](rejected-events.md) for real rejection data since found in a separate, wider capture, and the not-yet-implemented plan to act on it; live consumption behaviour; the audit topic's actual retention.
