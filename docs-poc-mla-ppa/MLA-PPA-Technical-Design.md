...
**Not resolved by the captures, and worth naming here rather than losing in a checklist:** no error, abort, or rejection was captured — all five transactions settle successfully. Every error-handling row in §2.6 and §3.7 remains built against the FSD specification alone.
...
| — | **No error, abort, or rejection captured.** All five transactions in `DRPP_Kafka_E2E_Pack` settle successfully. | Every error-path row in §2.6 and §3.7 remains built against the specification alone, unvalidated against real data. |
...
What the captures did **not** establish, and this design still relies on the FSD alone for: any error, abort, or rejection path (§2.6, §3.7); live consumption behaviour; the audit topic's actual retention.
