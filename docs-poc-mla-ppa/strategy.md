...
**Current state:** every tier of planned work is closed and live-verified against a real local Tazama TMS and ValKey. 199 automated tests (33 MLA + 166 PPA), 0 lint errors. One item remains genuinely blocked (error-path translation, blocked on COMESA supplying rejected-transaction captures); a short list of deployment-stage work (mTLS, the Keycloak token chain, Kubernetes manifests, full PII tokenization) is deliberately out of scope with no local environment to validate it against.
...
| **Reproducing or extending a live verification** | `MLA-PPA-Technical-Design.md` §7.1–§7.13 → newest `continue/` doc §4 (environment + the two checked-in tools) |
...
| **Capture pack** | `/home/abdul-rahim/mojaloop/DRPP_Kafka_E2E_Pack 2/DRPP_Kafka_E2E_Pack/` | Ground truth for the audit topic. Five per-transaction folders plus `raw_topic_slice_partition2.json`, a contiguous unfiltered partition read. `04_ZMW_to_EGP_partition_split` is the confirmed out-of-order case. |
| **Regression fixture** | `poc-mla-ppa/mla/__tests__/fixtures/raw_topic_slice_partition2.json` | The interleaved slice, checked in — the only fixture exercising the anchor-chaining maps across more than one transaction sharing a process lifetime. |
...
- **The error path is unverified by data, not unbuilt by omission.** The capture pack contains zero rejected, aborted or errored transactions. Do not start speculative work against a message shape nobody has observed.
